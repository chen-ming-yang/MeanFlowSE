...
class DiTBlockWithCrossAttention(nn.Module):
    def __init__(self, hidden_dim: int, heads: int, ff_dim: int, dropout: float):
        ...
        self.adaln_self_attn = nn.AdaLayerNorm(hidden_dim)

        self.self_attn = nn.MultiheadAttention(
            hidden_dim, heads, dropout=dropout, batch_first=True)
        
        self.adaln_cross_attn = nn.AdaLayerNorm(hidden_dim)
        self.cross_attn_norm_kv = nn.LayerNorm(hidden_dim)
        self.cross_attn = nn.MultiheadAttention(
            hidden_dim, heads, dropout=dropout, batch_first=True)
        
        self.adaln_ff = nn.AdaLayerNorm(hidden_dim)
    ...
    def forward(self, x, cond_seq, time_cond):
        # Self-attention
        h = self.adaln_self_attn(x, time_cond)
        attn_out, _ = self.self_attn(h, h, h)
        x = x + attn_out

        # Cross-attention to bottleneck
        # FFN
        h = self.adaln_cross_attn(x, time_cond)
        kv = self.cross_attn_norm_kv(cond_seq)
        cross_out, _ = self.cross_attn(h, kv, kv)
        x = x + cross_out

        # FFN
        h = self.adaln_ff(x, time_cond)
        x = x + self.feadforward(h)
        return x
...
class DirectMappingDiTBackbone(nn.Module):
    ...
    def __init__(
        ...
        self.pos_enc = SinusPositionEmbedding(hidden_dim)
...
class OneToManyDysarthriaSE(nn.Module):
    ...
    def _masked_adaptive_l2_loss(self, u_hat, u_target, mask=None):
        ...
        """
        # Compute squared error per token: (B, T, D) -> (B, T)
        delta_sq_full = (u_hat - u_target).pow(2).sum(dim=-1)
    ...
    def forward_train(self, batch):
        ...
        gt_length = vae_lengths.float()
        length_loss = F.mse_loss(pred_length, gt_length)
        ...
        total_loss = flow_loss + self.length_loss_weight * length_loss

        metrics = {
            "total_loss": total_loss.item(),
            "flow_loss": flow_loss.item(),
            "length_loss": length_loss.item(),
            "pred_length": pred_length.mean().item(),
            "gt_length": gt_length.mean().item(),
        }
