...
# Mean-Flow Speech Enhancement backbone


class DiTBackbone(nn.Module):
    """
    Adapter backbone for MeanFlowSE.

    Replaces the TTS-oriented DiT interface (mel + text tokens) with a
    speech-enhancement interface:
      forward(z_t, z_y, r, t) -> velocity field u_hat
    ...
    def __init__(
        ...
        # Fuse noisy latent and SSL conditioning into transformer dim
        self.input_proj = nn.Linear(latent_dim + ssl_dim, hidden_dim)

        # Sinusoidal sequence position embedding
        self.pos_embed = SinusPositionEmbedding(hidden_dim)

        # Sinusoidal time embeddings for t and r (flow ratio)
        self.t_embed = TimestepEmbedding(hidden_dim)
        self.r_embed = TimestepEmbedding(hidden_dim)
        ...
        self.norm_out = AdaLayerNorm_Final(hidden_dim)
        self.proj_out = nn.Linear(hidden_dim, latent_dim)

        # Zero-init output layers so the model starts as identity at t=0
    ...
    def forward(
        self,
        z_t: float["b n latent_dim"],  # noisy latent at time t
        z_y: float["b n ssl_dim"],     # SSL conditioning features
        r: float["b"],                 # flow start time (ratio)
        t: float["b"],                 # current time step
    ) -> float["b n latent_dim"]:
        # Concatenate noisy latent and SSL condition, then project
        x = self.input_proj(torch.cat([z_t, z_y], dim=-1))  # b n hidden_dim
        seq_len = x.shape[1]
        x = x + self.pos_embed(torch.arange(seq_len, device=x.device).float()).unsqueeze(0)

        # Positional and time conditioning
        rope = self.rotary_embed.forward_from_seq_len(seq_len)
        time_emb = self.t_embed(t) + self.r_embed(r)  # b hidden_dim

        for block in self.transformer_blocks:
            x = block(x, time_emb, rope=rope)
